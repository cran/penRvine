distr.func.help <- function(base,knots,penden.env,q,y) {
  ...
  if(q==3) {
    for(j in 1:(length(knots)-1)) {
      x.help <- c(x.help,(knots[j+1]-knots[j])/3+knots[j],knots[j]+2*(knots[j+1]-knots[j])/3,knots[j+1])
    }
  }
  if(q==4) {
    for(j in 1:(length(knots)-1)) {
      x.help <- c(x.help,(knots[j+1]-knots[j])/4+knots[j],knots[j]+2*(knots[j+1]-knots[j])/4,knots[j]+3*(knots[j+1]-knots[j])/4,knots[j+1])
    }
  }
  knots.help <- sort(unique(c(x.help,knots)))
  base.help <- my.bspline(y=knots.help, K =get("K",penden.env),q=q,kn=knots)$base.den
...
for(j in 1:len.b) {
  ...
  #print(j)
    for(i in 1:(len.k-1)) {
      ...
      if(q==4) y.vec <- knots.help[c(a,a+(b-a)/4,a+2*(b-a)/4,a+3*(b-a)/4,b)]
